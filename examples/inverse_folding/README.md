...
conda install pip
pip install biotite
pip install git+https://github.com/facebookresearch/fair_esm.git
```
...
```
import fair_esm.inverse_folding
structure = fair_esm.inverse_folding.util.load_structure(fpath, chain_id)
coords, seq = fair_esm.inverse_folding.util.extract_coords_from_structure(structure)
```
...
```
structure = fair_esm.inverse_folding.util.load_structure(fpath, chain_ids)
...
```
ll_fullseq, ll_withcoord = fair_esm.inverse_folding.util.score_sequence(model, alphabet, coords, seq)
```
...
```
ll_fullseq, ll_withcoord = fair_esm.inverse_folding.multichain_util.score_sequence_in_complex(
...
```
rep = fair_esm.inverse_folding.util.get_encoder_output(model, alphabet, coords)
```
...
```
rep = fair_esm.inverse_folding.multichain_util.get_encoder_output_for_complex(
